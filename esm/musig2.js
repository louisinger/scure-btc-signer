...
import { aInRange, bytesToNumberBE, concatBytes, equalBytes, numberToBytesBE, } from '@noble/curves/abstract/utils';
...
const compressed = P.apply(P.bytes(33), {
    ...
    encode: (b) => (equalBytes(b, ZERO) ? Point.ZERO : Point.fromHex(b)),
});
...
const modN = (x) => mod(x, SECP_N);
const taggedInt = (tag, ...messages) => modN(bytesToNumberBE(taggedHash(tag, ...messages)));
